...
const PLAYBOOKS: Record<Subject, SubjectPlaybook> = {
  ...
  chinese: {
    label: "國語",
    focus:
      "把照片裡可以辨認的中文字、生字、詞語、句子、注音**全部**找出來；如果是課本/作業也請一起讀懂題目。生字頁有幾個字就要教幾個字，絕對不能只挑一兩個。",
    style:
      "像國語老師一樣，先認字（含注音）、再說意思、再造個簡單例句；遇到成語或句型要解釋",
    sectionHint:
      "兩段：第一段「認字 / 詞語小教室」（簡介這頁主題，並逐一說明每個生字/詞語），第二段「造句或語感練習」；單字段請改名為「重要詞語」，**逐字列出照片中看到的每一個生字/詞語**（注音 — 詞 — 意思），照片有 N 個就列 N 個",
  },
  english: {
    label: "英文",
    focus:
      "把照片裡可以辨認的英文字、單字、片語、句子、對話**全部**找出來；單字頁有幾個單字就要教幾個",
    style:
      "像英文老師，先念音再翻譯，遇到單字一定要附 KK 音標，並用一兩句中文解釋；給一個跟生活有關的例句",
    sectionHint:
      "兩段：第一段「Words & Pronunciation 單字與發音」（**逐一**列出照片中所有單字，配 KK 音標與中文意思，照片有 N 個就列 N 個），第二段「Talk Like This 試著這樣說」",
  },
  math: {
    label: "數學",
    focus:
...
- **照片裡所有可辨認的生字／單字／詞語都要逐一列入「重要詞彙」段落，不可只挑一兩個。**例如生字頁有 15 個生字就要列 15 個
- 段落本文字數控制在 300~500 字（**不含**「重要詞彙」清單；清單想列幾個就幾個，不算進字數）
- 風格要溫柔、鼓勵、像跟小朋友說話
- 段落結構：${pb.sectionHint}
...
2. 除了讀取上面那幾個照片檔之外，不要做其他事情（不要上網、不要編輯/搬移檔案、不要執行 shell 命令）
3. 不要加額外的章節，照骨架就好
4. ${TC_REMINDER}`;
}
